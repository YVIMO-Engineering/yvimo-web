...
begin
  ...
  if v_order.manufacturing_type = 'multi-step' and not exists (
    ...
    where serial.organization_id = p_organization_id
      ...
      and serial.assigned_station = v_station_code
      and serial.result is null
  ) then
    raise exception 'This Multi-step order has no pending pieces assigned to the selected station.';
  end if;
  ...
  insert into public.mes_operator_terminal_events (
    ...
    jsonb_build_object(
      ...
      'shift', p_shift,
      'manufacturing_type', v_order.manufacturing_type
    )
  );
