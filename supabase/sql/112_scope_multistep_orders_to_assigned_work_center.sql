...
begin
  ...
  where station.organization_id = p_organization_id
    ...
    and (
      v_order.manufacturing_type <> 'multi-step'
      or work_center.code = v_order.assigned_work_center
    )
  limit 1;

  if v_work_center_code is null then
    if v_order.manufacturing_type = 'multi-step' then
      raise exception 'This Multi-step order belongs to a different Work Center.';
    end if;
  ...
  where organization_id = p_organization_id
    ...
    and work_center_id in (
      select id from public.mes_work_centers
      where organization_id = p_organization_id and code = v_work_center_code
    );
