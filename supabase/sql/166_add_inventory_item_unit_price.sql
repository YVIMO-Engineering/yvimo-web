alter table public.mes_inventory_items
  add column if not exists unit_price numeric(14, 4) not null default 0 check (unit_price >= 0),
  add column if not exists currency text not null default 'USD' check (length(btrim(currency)) > 0);

comment on column public.mes_inventory_items.unit_price is
  'Price of a single unit of the item. Profit Leak values Supplies Used events with this price.';
comment on column public.mes_inventory_items.currency is
  'Currency of unit_price.';

-- Stamp the unit price on every inventory movement so Profit Leak keeps the
-- historical cost of the event even if the item price changes later.
...
begin
  ...
  -- Only priced items carry cost keys; a zero price must stay "not recorded".
  if coalesce(v_item.unit_price, 0) > 0 then
    v_payload = v_payload || jsonb_build_object(
      'unit_price', v_item.unit_price,
      'unit_cost', v_item.unit_price,
      'total_cost', v_item.unit_price * abs(p_delta),
      'currency', coalesce(nullif(btrim(v_item.currency), ''), 'USD')
    );
  end if;
