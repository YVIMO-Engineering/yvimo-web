-- Rework orders never start in the green.
--
-- An RW- order exists because something already went wrong on a regular order, so it is
-- more urgent than a regular order with the same delivery date. Production Schedule now
-- paints an RW- order at least moderate risk (yellow) and Intelligent Scheduling ranks it
-- the same way: a rework order that would be low risk by date alone takes rank 3 instead
-- of 4. Overdue and high-risk rework orders keep their rank, and quarantine still wins.
--
-- Only the rank case changes; the rest of the function is migration 182 as it was.
