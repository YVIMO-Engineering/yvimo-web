...
create index if not exists mes_production_serials_assigned_station_idx
  on public.mes_production_serials (organization_id, assigned_station)
  where assigned_station is not null;

comment on column public.mes_production_serials.assigned_station is
  'Station assigned to this individual serialized piece for multi-step production orders.';
...
begin
  ...
  v_station_code = coalesce(nullif(btrim(new.assigned_station), ''), nullif(btrim(v_order.assigned_station), ''));
  if v_station_code is null then return new; end if;

  select cycle.*
    into v_active_cycle
  from public.mes_station_status_cycles cycle
  where cycle.organization_id = new.organization_id
    and cycle.station_code = v_station_code
  ...
  where serial.production_order_id = v_order.id
    ...
    and coalesce(nullif(btrim(serial.assigned_station), ''), nullif(btrim(v_order.assigned_station), '')) = v_station_code
