-- Intelligent Scheduling also levels multi-step work.
--
-- Migration 180 moved only single-operation orders between mirror machines, because a
-- multi-step card counts the pieces assigned to its own station and moving the card
-- alone would leave it reading "0 pieces for this station". That left the mirror pairs
-- lopsided: one Gemini carrying every multi-step order while its sister ran two cards.
--
-- Now the pieces travel with the card. When the balancer moves a multi-step order to a
-- sister machine, the pending pieces assigned to the machine it leaves are reassigned to
-- the machine it lands on, which is exactly what mirror machines mean. An order whose
-- pieces are already split across both machines of a pool stays put: that split is the
-- balance, and shuffling it would only move work for nothing.
