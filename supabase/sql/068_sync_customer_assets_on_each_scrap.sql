create or replace function public.sync_mes_scrap_serial_asset(p_serial_id uuid)
returns void
language plpgsql
security definer
...
begin
  ...
  where id = p_serial_id and result = 'scrap';
  ...
  insert into public.mes_customer_asset_service_events (
    ...
    'Sharpening', 'scrap', coalesce(v_serial.reported_at, now()),
    format('Scrap reported live through Production Order %s.', v_order.order_number)
  )
  on conflict do nothing
...
revoke all on function public.sync_mes_scrap_serial_asset(uuid) from public;

create or replace function public.sync_mes_scrap_serial_asset_trigger()
returns trigger
language plpgsql
security definer
set search_path = public
as $$
begin
  if new.result = 'scrap' and (tg_op = 'INSERT' or old.result is distinct from new.result) then
    perform public.sync_mes_scrap_serial_asset(new.id);
  end if;
  return new;
end;
$$;

drop trigger if exists sync_mes_scrap_serial_asset on public.mes_production_serials;
create trigger sync_mes_scrap_serial_asset
after insert or update of result on public.mes_production_serials
for each row execute function public.sync_mes_scrap_serial_asset_trigger();

-- Bring historical serialized Scrap reports into the Asset history. Existing
-- uniqueness constraints prevent duplicates when an Asset/event is present.
do $$
declare v_serial record;
begin
  for v_serial in select id from public.mes_production_serials where result = 'scrap' loop
    perform public.sync_mes_scrap_serial_asset(v_serial.id);
  end loop;
end;
$$;
