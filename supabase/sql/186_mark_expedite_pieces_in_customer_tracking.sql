-- Customer Portal Production Tracking marks the pieces that ran as expedites.
--
-- A portal user cannot read mes_expedite_tool_ids, so the tracking function answers the
-- question itself: is_expedite is true when the Tool ID shown on the row is registered in
-- Expedite Orders, paused or not, compared the same trimmed, case-insensitive way the
-- registry's unique index does. Only the flag leaves the function, never the rule.
--
-- The return type changes, so the function is dropped and created again; the body is
-- migration 159 plus the new column.
...
select o.id,o.order_number,r.received_at,o.piece_type,o.part_number,o.status,s.id,s.piece_sequence,
 ...
 exists(select 1 from public.mes_expedite_tool_ids e where e.organization_id=o.organization_id and lower(btrim(e.tool_id))=lower(coalesce(nullif(btrim(s.tool_id),''),nullif(btrim(t.tool_id),''))))
