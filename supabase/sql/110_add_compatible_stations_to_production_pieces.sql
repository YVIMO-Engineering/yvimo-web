alter table public.mes_production_serials
  add column if not exists compatible_stations text[] not null default '{}'::text[];

update public.mes_production_serials
set compatible_stations = array[assigned_station]
where assigned_station is not null
  and btrim(assigned_station) <> ''
  and cardinality(compatible_stations) = 0;

create index if not exists mes_production_serials_compatible_stations_idx
  on public.mes_production_serials using gin (compatible_stations);

comment on column public.mes_production_serials.compatible_stations is
  'Stations where this serialized piece may be processed. assigned_station remains the primary or actual station.';
...
begin
  ...
  if v_work_center_code is null then raise exception 'Selected station was not found.'; end if;
  ...
  if v_order.manufacturing_type = 'multi-step' then
    update public.mes_production_orders set status = 'paused'
    where id = v_order.id returning * into v_order;
  else
    update public.mes_production_orders
    set status = 'paused', assigned_work_center = v_work_center_code, assigned_station = v_station_code
    where id = v_order.id returning * into v_order;
  end if;
  ...
  where organization_id = p_organization_id and code = v_station_code;
