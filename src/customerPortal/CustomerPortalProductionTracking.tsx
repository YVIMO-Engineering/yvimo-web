...
import { ChevronDown, ClipboardList, LoaderCircle, Search, Siren } from 'lucide-react';
...
type TrackingRow = {
  ...
  // Added by migration 186; absent while the database still runs the older function.
  is_expedite?: boolean | null;
};
...
export function CustomerPortalProductionTracking({ organizationId, customerId }: Props) {
  ...
  const [from, setFrom] = React.useState(() => inputDate(new Date(now.getFullYear(), 0, 1)));
  const [to, setTo] = React.useState(() => inputDate(new Date(now.getFullYear(), 11, 31)));
