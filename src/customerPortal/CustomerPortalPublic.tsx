...
import { CustomerPortalNotifications } from './CustomerPortalNotifications';
import { useCustomerPortalNotifications } from './useCustomerPortalNotifications';
...
export function CustomerPortalPublic({ user, supplierOrganization, onSignOut }: Props) {
  ...
  const notificationFeed = useCustomerPortalNotifications(portalContext?.organizationId ?? '', portalContext?.customerId ?? '', user.id);
  ...
  return <main className="cp-public-shell">
    <aside className={mobileOpen ? 'open' : ''}>
      ...
      <nav>{navigation.map((item) => <button className={active === item.label ? 'active' : ''} key={item.label} onClick={() => { setActive(item.label); setMobileOpen(false); }}><item.icon size={18} /><span>{item.label}</span>{item.label === 'Notifications' && notificationFeed.unread ? <em>{notificationFeed.unread}</em> : null}</button>)}</nav>
    ...
    <section className="cp-public-main">
      <header><button className="cp-mobile-menu" onClick={() => setMobileOpen(true)}><Menu size={20} /></button><div><small>{new Intl.DateTimeFormat('en', { weekday: 'long', month: 'long', day: '2-digit' }).format(new Date()).toUpperCase()}</small><strong>Good morning, {user.name.split(' ')[0]}</strong></div><div className="cp-public-header-actions"><button><Search size={19} /></button><button className={notificationFeed.unread ? 'has-alert' : ''} title="Notifications" onClick={() => setActive('Notifications')}><Bell size={19} /></button><button onClick={onSignOut} title="Sign out"><LogOut size={18} /></button></div></header>
      {active === 'Dashboard' ? <CustomerPortalDashboard organizationId={portalContext.organizationId} customerId={portalContext.customerId} supplierName={supplierName} onOpenOrders={() => setActive('My Active Orders')} /> : active === 'My Active Orders' ? <CustomerPortalDashboard view="orders" organizationId={portalContext.organizationId} customerId={portalContext.customerId} supplierName={supplierName} onOpenOrders={() => undefined} /> : active === 'Tools' ? <CustomerPortalTools organizationId={portalContext.organizationId} customerId={portalContext.customerId} /> : active === 'Production Tracking' ? <CustomerPortalProductionTracking organizationId={portalContext.organizationId} customerId={portalContext.customerId} /> : active === 'Shipments' ? <CustomerPortalDashboard view="shipments" organizationId={portalContext.organizationId} customerId={portalContext.customerId} supplierName={supplierName} onOpenOrders={() => undefined} /> : active === 'Notifications' ? <CustomerPortalNotifications notifications={notificationFeed.notifications} unread={notificationFeed.unread} loading={notificationFeed.loading} error={notificationFeed.error} onAcknowledge={notificationFeed.acknowledge} /> : <div className="cp-section-placeholder"><span><Settings size={28} /></span><small>CUSTOMER PORTAL</small><h1>{active}</h1><p>This section is ready in the portal navigation. Its operational content and permissions will be connected in the next phase.</p><button onClick={() => setActive('Dashboard')}>Return to dashboard</button></div>}
    </section>
  </main>;
}
