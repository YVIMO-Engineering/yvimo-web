...
const maximumAvailableRecordNumber = 1000;
...
function getRecordNumberInRange(value: string): number | null {
  const match = value.trim().match(/^(?:\D*?)0*(\d{1,4})$/);
  if (!match) return null;
  const recordNumber = Number(match[1]);
  return recordNumber >= 0 && recordNumber <= maximumAvailableRecordNumber ? recordNumber : null;
}
...
export function HealthPatientsWorkspace({ organizationId, organizationName, onNavigate, t, languageCode }: Props) {
  ...
  const availableRecordNumbers = Array.from({ length: maximumAvailableRecordNumber + 1 }, (_, index) => index).filter((recordNumber) => !occupiedRecordNumbers.has(recordNumber));
  ...
  const savePatient = async (event: React.FormEvent) => {
    ...
    const fullName = normalizePatientName(form.fullName.trim().replace(/\s+/g, ' '));
