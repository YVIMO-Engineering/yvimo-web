...
type PendingWorkReportStation = {
  key: string;
  label: string;
  totalQuantity: number;
  lines: PendingWorkReportLine[];
  runningOrder: ProductionOrder | null;
};

type PendingWorkReport = {
  generatedAt: string;
  totalQuantity: number;
  totalOrders: number;
...
function getPendingWorkReportWorkCenterCode(
  orders: ProductionOrder[],
  workCenterOptions: MesOrderDropdownOption[],
  stationOptionsByWorkCenter: Record<string, MesOrderDropdownOption[]>,
) {
  const gleasonWorkCenter = workCenterOptions.find((option) => /gleason\s+norte/i.test(option.label));
  if (gleasonWorkCenter?.value && stationOptionsByWorkCenter[gleasonWorkCenter.value]?.length) return gleasonWorkCenter.value;
  const workCenterFromOrders = orders.find((order) => order.assignedWorkCenter && stationOptionsByWorkCenter[order.assignedWorkCenter]?.length)?.assignedWorkCenter;
  if (workCenterFromOrders) return workCenterFromOrders;
  return Object.entries(stationOptionsByWorkCenter).find(([, stations]) => stations.length)?.[0] ?? '';
}

function getPendingWorkPartLabel(partName: string, quantity: number) {
  const normalizedPart = partName.trim().toLowerCase();
  if (!normalizedPart) return quantity === 1 ? 'piece' : 'pieces';
  if (/\bshaver\b|\bshavers\b/.test(normalizedPart)) return quantity === 1 ? 'shaver' : 'shavers';
  if (/\bhob\b|\bhobs\b/.test(normalizedPart)) return quantity === 1 ? 'hob' : 'hobs';
  if (/\bskiving\b|\bskivings\b/.test(normalizedPart)) return quantity === 1 ? 'skiving' : 'skivings';
  if (/\bpower\s*skiver\b|\bpowerskiver\b|\bpowerskivers\b/.test(normalizedPart)) return quantity === 1 ? 'powerskiver' : 'powerskivers';
  if (/\btallador\b|\btalladores\b/.test(normalizedPart)) return quantity === 1 ? 'tallador' : 'talladores';
  if (/\bshaper\b|\bshapers\b/.test(normalizedPart)) return quantity === 1 ? 'shaper' : 'shapers';
  return quantity === 1 ? normalizedPart : normalizedPart.endsWith('s') ? normalizedPart : `${normalizedPart}s`;
}

function getPendingWorkClientLabel(clientName: string) {
  const client = clientName.trim();
  if (!client) return 'Unassigned';
  if (/^caterpillar$/i.test(client)) return 'Cat';
  return client;
}

function formatPendingWorkPartDisplay(partLabel: string) {
  const normalizedPartLabel = partLabel.trim().toLowerCase();
  return normalizedPartLabel ? normalizedPartLabel.charAt(0).toUpperCase() + normalizedPartLabel.slice(1) : 'Pieces';
}

function getPendingWorkPartTone(partLabel: string) {
  const normalizedPartLabel = partLabel.trim().toLowerCase();
  if (normalizedPartLabel.includes('shaver')) return 'shaver';
  if (normalizedPartLabel.includes('hob')) return 'hob';
  if (normalizedPartLabel.includes('skiving') || normalizedPartLabel.includes('powerskiver')) return 'skiving';
  if (normalizedPartLabel.includes('tallador')) return 'tallador';
  if (normalizedPartLabel.includes('shaper')) return 'shaper';
  return 'default';
}

function getPendingWorkCategoryLabel(category: PendingWorkCategory) {
...
function getPendingWorkReport(
  ...
  pendingEntries.forEach(({ order, stationCode, category, quantity }) => {
    ...
    if (order.status === 'running' && !station.runningOrder) station.runningOrder = order;
    if (existingLine) {
    ...
    stationMap.set(stationLabel, station);
  });
  ...
  const subtotals = pendingEntries.reduce<Record<PendingWorkCategory, number>>((totals, entry) => ({
    ...totals,
  ...
  return {
    ...
    totalQuantity,
    totalOrders: new Set(pendingEntries.map((entry) => entry.order.id)).size,
    totalMachines: reportMachineOptions.length,
    subtotals,
    stations,
    machineStatuses,
    text: reportLines.join('\n').trim(),
...
function PendingWorkReportModal({
  ...
  const modalContent = (
    <div className="mes-modal-backdrop pending-work-report-backdrop" role="presentation">
      <section className="pending-work-report-modal" role="dialog" aria-modal="true" aria-labelledby="pending-work-report-title">
        <header className="pending-work-report-header">
          <span><FileText size={22} /></span>
          <div>
            <p className="eyebrow">Production Orders</p>
            <h3 id="pending-work-report-title">Pending Work Report</h3>
            <small>Generated {formatTimestamp(report.generatedAt)}</small>
          </div>
          <div className="pending-work-report-header-actions">
            <button className="pending-work-report-copy-action" type="button" onClick={() => void copyReport()}>
              <Copy size={16} /> {copied ? 'Copied' : 'Copy Shareable Text'}
            </button>
            <button className="pending-work-report-pdf-action" type="button" disabled={downloadingPdf} onClick={() => void downloadReportPdf()}>
              <Download size={16} /> {downloadingPdf ? 'Generating PDF' : 'Download PDF'}
            </button>
            <button className="pending-work-report-close" type="button" aria-label="Close pending work report" onClick={onClose}><X size={18} /></button>
          </div>
        </header>
        ...
        <div className="pending-work-report-body">
          <section className="pending-work-report-panel pending-work-report-pending" aria-label="Pending work by machine">
            ...
            {visibleReport.stations.length ? (
                ...
                {visibleReport.stations.map((station) => (
                  <article className="pending-work-report-station" key={station.key}>
                    <div>
                      ...
                      <span>{station.totalQuantity.toLocaleString()} pending</span>
                    </div>
                    <ul>
              ...
              <div className="pending-work-report-empty">
                <CheckCircle2 size={26} />
                <strong>No pending work found</strong>
                <span>All active Production Orders are currently reported or closed.</span>
              </div>
            )}
          </section>
...
export function WorkCentersWorkspace({ onNavigate, organizationId }: WorkspaceProps) {
  const workCenterSelectionKey = `yvimo:work-centers:selected:${organizationId}`;
  const stationSelectionKey = `yvimo:work-centers:selected-station:${organizationId}`;
  ...
  const [stationLoadEvents, setStationLoadEvents] = React.useState<StationLoadEvent[]>([]);
  ...
  const [cycleDetailSelection, setCycleDetailSelection] = React.useState<{ stationId: string; status: WorkCenterStatus } | null>(null);
  const [cycleClock, setCycleClock] = React.useState(() => Date.now());
  const [selectedWorkCenterId, setSelectedWorkCenterId] = React.useState(() => window.localStorage.getItem(workCenterSelectionKey) ?? '');
  const [selectedStationId, setSelectedStationId] = React.useState(() => window.localStorage.getItem(stationSelectionKey) ?? '');
  ...
  const loadWorkCenters = React.useCallback(async (silent = false) => {
    ...
    if (workCenterError || stationError || productionOrderError || productionEventError || stationLoadEventError || statusCycleError || serialStationError) {
      setWorkCentersError(workCenterError?.message ?? stationError?.message ?? productionOrderError?.message ?? productionEventError?.message ?? stationLoadEventError?.message ?? statusCycleError?.message ?? serialStationError?.message ?? 'Could not load Work Centers.');
      if (silent) return;
      setWorkCenters([]);
      setProductionOrders([]);
      setProductionEvents([]);
      setStationLoadEvents([]);
    ...
    setStationLoadEvents((stationLoadEventRows as StationLoadEvent[] | null) ?? []);
  ...
  React.useEffect(() => {
    if (selectedWorkCenterId) window.localStorage.setItem(workCenterSelectionKey, selectedWorkCenterId);
  }, [selectedWorkCenterId, workCenterSelectionKey]);

  React.useEffect(() => {
    if (selectedStationId) window.localStorage.setItem(stationSelectionKey, selectedStationId);
    else window.localStorage.removeItem(stationSelectionKey);
  }, [selectedStationId, stationSelectionKey]);
  ...
  const historicalStationCodesByOrder = stationLoadEvents.reduce<Record<string, string[]>>((groups, event) => {
    if (!event.production_order_id || !event.station_code) return groups;
    const codes = groups[event.production_order_id] ?? [];
    if (!codes.includes(event.station_code)) codes.push(event.station_code);
    groups[event.production_order_id] = codes;
    return groups;
  }, {});
  const stationLoadRows = calculateStationLoads({
    stations: selectedStations,
    orders: productionOrders,
    events: stationLoadEvents,
    stationCodesForOrder: (order) => {
      if (order.assignedWorkCenter !== selectedWorkCenter?.code) return [];
      if (order.manufacturingType === 'multi-step') {
        return ['completed', 'cancelled'].includes(order.status)
          ? historicalStationCodesByOrder[order.id] ?? []
          : multiStepStationsByOrder[order.id] ?? [];
      }
      return order.assignedStation ? [order.assignedStation] : [];
    },
  });
  const workCenterCurrentLoadPoints = stationLoadRows.reduce((total, station) => total + station.currentLoadPoints, 0);
  const workCenterReferenceLoadPoints = stationLoadRows.reduce((total, station) => total + station.referenceLoadPoints, 0);
  const workCenterLoadPercent = workCenterReferenceLoadPoints > 0
    ? Math.round((workCenterCurrentLoadPoints / workCenterReferenceLoadPoints) * 100)
    : 0;
  const workCenterLoadStatus = getStationLoadStatus(workCenterLoadPercent);
  const workCenterGaugeRatio = Math.min(workCenterLoadPercent, 120) / 120;
  const workCenterGaugeAngle = Math.PI - (Math.PI * workCenterGaugeRatio);
  const workCenterGaugeNeedleX = 120 + (Math.cos(workCenterGaugeAngle) * 61);
  const workCenterGaugeNeedleY = 108 - (Math.sin(workCenterGaugeAngle) * 61);
  ...
  const getCycleContext = (cycle: StationStatusCycleRow | null) => {
    if (!cycle) return { orderNumber: '—', serialNumber: '—', clientName: '—' };
    const order = cycle.production_order_id
      ? productionOrders.find((candidate) => candidate.id === cycle.production_order_id)
      : productionOrders.find((candidate) => candidate.orderNumber === cycle.order_number);
    return {
      orderNumber: cycle.order_number || order?.orderNumber || '—',
      serialNumber: cycle.serial_number || '—',
      clientName: cycle.client_name || order?.clientName || '—',
    };
  };
  ...
  const downloadCycleReportPdf = async () => {
    ...
    try {
      ...
      const margin = 42;
      const pageHeight = pdf.internal.pageSize.getHeight();
      const pdfCell = (value: string, maxLength: number) => value.length > maxLength ? `${value.slice(0, Math.max(1, maxLength - 3))}...` : value;
      let cursorY = margin;
      const ensureSpace = (height: number) => {
        if (cursorY + height <= pageHeight - margin) return;
        pdf.addPage();
      ...
      pdf.setFont('helvetica', 'bold');
      pdf.setFontSize(9);
      pdf.text('WORK CENTER TOTALS', margin, cursorY);
      cursorY += 13;
      pdf.setFont('helvetica', 'normal');
      (['running', 'idle', 'down', 'setup', 'maintenance', 'offline'] as WorkCenterStatus[]).forEach((status) => {
        pdf.text(`${formatLabel(status)}: ${formatCycleDuration(cycleTotals[status] ?? 0)}`, margin, cursorY);
        cursorY += 12;
      });
      cursorY += 12;
      const pdfStatusColors: Record<string, [number, number, number]> = {
        running: [154, 87, 0],
        idle: [0, 124, 91],
        down: [185, 28, 28],
        setup: [109, 40, 217],
        maintenance: [154, 87, 0],
        offline: [71, 85, 105],
      };
      selectedStations.forEach((station, stationIndex) => {
        if (stationIndex > 0) {
          pdf.addPage();
          cursorY = margin;
        }
        ...
        const stationTracked = stationCycles.reduce((total, cycle) => total + reportCycleDuration(cycle), 0);
        ensureSpace(76);
        const stationHeaderHeight = 30;
        const stationHeaderWidth = pdf.internal.pageSize.getWidth() - (margin * 2);
        pdf.setFillColor(255, 247, 237);
        pdf.setDrawColor(255, 122, 0);
        pdf.setLineWidth(1);
        pdf.roundedRect(margin, cursorY - 13, stationHeaderWidth, stationHeaderHeight, 5, 5, 'FD');
        pdf.setFont('helvetica', 'bold');
        pdf.setFontSize(12);
        pdf.setTextColor(255, 122, 0);
        pdf.text(`${station.name}  ${station.code}`, margin + 12, cursorY + 5);
        pdf.setFontSize(9);
        pdf.setTextColor(154, 74, 0);
        pdf.text(
          `${formatCycleDuration(stationTracked)} tracked`,
          margin + stationHeaderWidth - 12,
          cursorY + 5,
          { align: 'right' },
        );
        cursorY += 29;
        ...
        pdf.text('LAST STARTED', margin + 70, cursorY);
        pdf.text('LAST ENDED', margin + 170, cursorY);
        pdf.text('ORDER', margin + 270, cursorY);
        pdf.text('SERIAL NUMBER', margin + 335, cursorY);
        pdf.text('CLIENT', margin + 420, cursorY);
        pdf.text('TOTAL TIME', margin + 550, cursorY);
        pdf.text('CYCLES', margin + 635, cursorY);
        cursorY += 12;
        (['running', 'idle', 'down', 'setup', 'maintenance', 'offline'] as WorkCenterStatus[]).forEach((status) => {
          const statusCycles = stationCycles
            .filter((cycle) => cycle.status === status)
            .sort((left, right) => new Date(left.started_at).getTime() - new Date(right.started_at).getTime());
          const lastCycle = statusCycles.at(-1) ?? null;
          const lastStartedAt = lastCycle ? Math.max(new Date(lastCycle.started_at).getTime(), cycleReportRangeStart) : null;
          const lastEndedAt = lastCycle ? Math.min(lastCycle.ended_at ? new Date(lastCycle.ended_at).getTime() : cycleClock, cycleReportRangeEnd) : null;
          const lastCycleContext = getCycleContext(lastCycle);
          ensureSpace(statusCycles.length ? 30 + (statusCycles.length * 12) : 18);
          const statusColor = pdfStatusColors[status] ?? [20, 32, 45];
          ...
          pdf.setFont('helvetica', 'normal');
          pdf.setFontSize(7.5);
          pdf.text(lastStartedAt === null ? '-' : formatTimestamp(new Date(lastStartedAt).toISOString()), margin + 70, cursorY);
          pdf.text(lastEndedAt === null ? '-' : lastCycle && !lastCycle.ended_at && cycleReportDateRange.to === getMesOrderQuickRange('today').to ? 'Current' : formatTimestamp(new Date(lastEndedAt).toISOString()), margin + 170, cursorY);
          pdf.text(pdfCell(lastCycleContext.orderNumber, 11), margin + 270, cursorY);
          pdf.text(pdfCell(lastCycleContext.serialNumber, 14), margin + 335, cursorY);
          pdf.text(pdfCell(lastCycleContext.clientName, 22), margin + 420, cursorY);
          pdf.setFont('helvetica', 'bold');
          pdf.text(formatCycleDuration(statusTotals[status] ?? 0), margin + 550, cursorY);
          pdf.text(String(statusCycles.length), margin + 635, cursorY);
          cursorY += 13;

          if (statusCycles.length) {
            pdf.setFont('helvetica', 'bold');
            pdf.setFontSize(7.5);
            pdf.setTextColor(...statusColor);
            pdf.text('ALL CYCLES', margin + 18, cursorY);
            cursorY += 10;
            pdf.setTextColor(83, 100, 118);
            pdf.setFontSize(6.5);
            pdf.text('#', margin + 18, cursorY);
            pdf.text('STARTED', margin + 38, cursorY);
            pdf.text('ENDED', margin + 140, cursorY);
            pdf.text('ORDER', margin + 242, cursorY);
            pdf.text('SERIAL NUMBER', margin + 307, cursorY);
            pdf.text('CLIENT', margin + 392, cursorY);
            pdf.text('DURATION', margin + 550, cursorY);
            cursorY += 9;
            statusCycles.forEach((cycle, index) => {
              ensureSpace(14);
              const startedAt = Math.max(new Date(cycle.started_at).getTime(), cycleReportRangeStart);
              const endedAt = Math.min(cycle.ended_at ? new Date(cycle.ended_at).getTime() : cycleClock, cycleReportRangeEnd);
              const cycleContext = getCycleContext(cycle);
              const endedLabel = !cycle.ended_at && cycleReportDateRange.to === getMesOrderQuickRange('today').to
                ? 'Current'
                : formatTimestamp(new Date(endedAt).toISOString());
              pdf.setFont('helvetica', 'normal');
              pdf.setFontSize(7);
              pdf.setTextColor(52, 71, 90);
              pdf.text(`#${index + 1}`, margin + 18, cursorY);
              pdf.text(formatTimestamp(new Date(startedAt).toISOString()), margin + 38, cursorY);
              pdf.text(endedLabel, margin + 140, cursorY);
              pdf.text(pdfCell(cycleContext.orderNumber, 11), margin + 242, cursorY);
              pdf.text(pdfCell(cycleContext.serialNumber, 14), margin + 307, cursorY);
              pdf.text(pdfCell(cycleContext.clientName, 25), margin + 392, cursorY);
              pdf.text(formatCycleDuration(Math.max(0, endedAt - startedAt)), margin + 550, cursorY);
              cursorY += 11;
            });
          }
          cursorY += 5;
  ...
  const openEditWorkCenterForm = (workCenter: MesWorkCenter) => {
    selectWorkCenter(workCenter.id);
    setFormState(workCenterToFormState(workCenter));
    setEditingWorkCenterId(workCenter.id);
    setAddressLookup({
      status: 'success',
      message: `Location found: ${workCenter.latitude.toFixed(5)}, ${workCenter.longitude.toFixed(5)}`,
    });
    setAddressSuggestions([]);
    setShowAddressSuggestions(false);
    setShowAddForm(true);
  ...
  const deleteWorkCenter = async (workCenter: MesWorkCenter) => {
    const deletingWorkCenterId = workCenter.id;
  ...
  const confirmDeleteWorkCenter = (workCenter: MesWorkCenter) => {
    selectWorkCenter(workCenter.id);
    setWorkCenterConfirmation({
      ...
      tone: 'danger',
      onConfirm: () => deleteWorkCenter(workCenter),
    });
  };
  ...
  React.useEffect(() => {
    if (!showAddForm && !showStationForm && !showDetailModal && !stationLoadHelpOpen && !jobQueueSummary && !workCenterConfirmation) return undefined;
    const previousBodyOverflow = document.body.style.overflow;
    const previousDocumentOverflow = document.documentElement.style.overflow;
    document.body.style.overflow = 'hidden';
    document.documentElement.style.overflow = 'hidden';
  ...
  }, [showAddForm, showStationForm, showDetailModal, stationLoadHelpOpen, jobQueueSummary, workCenterConfirmation]);
  ...
  return (
    <section className={['mes-workspace-panel work-centers-workspace', workCenterMapExpanded ? 'map-expanded' : ''].filter(Boolean).join(' ')}>
        ...
        <main className="work-centers-operations-main">
          <section className="work-centers-management-panel">
            <div className="work-centers-panel-heading">
              ...
              <div className="work-centers-heading-controls">
                <div className="work-center-selected-actions">
                  <button type="button" disabled={!selectedWorkCenter} aria-label="Edit selected Work Center" title="Edit selected Work Center" onClick={() => { if (selectedWorkCenter) openEditWorkCenterForm(selectedWorkCenter); }}>
                    <Pencil size={15} aria-hidden="true" />
                  </button>
                  <button type="button" disabled={!selectedWorkCenter} aria-label="Delete selected Work Center" title="Delete selected Work Center" onClick={() => { if (selectedWorkCenter) confirmDeleteWorkCenter(selectedWorkCenter); }}>
                ...
                return (
                  ...
                  >
                    <span className="work-center-table-identity">
                      ...
                      <span className="work-center-table-name-block">
                        <span className="work-center-table-name-line"><strong>{workCenter.name}</strong></span>
                        <em>{workCenter.code}</em>
                      </span>
                    </span>
          ...
          <section className="stations-management-panel">
            ...
            <div className="station-insights-layout">
              ...
              <header>
                <div>
                  ...
                  <strong id="station-load-chart-title">Current workload by station</strong>
                </div>
                <div className="station-load-chart-help">
                  <small>Current workload by station · 100% = historical high-load reference</small>
                  <button type="button" aria-label="How Station Load is calculated" title="How Station Load is calculated" onClick={() => setStationLoadHelpOpen(true)}>
                ...
                {stationLoadRows.map((load) => (
                  <div className="station-load-chart-row" key={load.stationId}>
                    ...
                    <div
                      ...
                      role="meter"
                      aria-label={`${load.stationName} current workload`}
                      aria-valuemin={0}
                      aria-valuemax={Math.max(100, load.loadPercent)}
                      aria-valuenow={load.loadPercent}
                    >
                      <span
                        className={`load-${load.loadStatus}`}
                        style={{ width: `${load.visualLoadPercent}%` }}
                      />
                    </div>
                    <strong className="station-load-chart-value">{load.loadPercent}%</strong>
                    <span className={`machine-load-pill load-${load.loadStatus}`}>{formatTitleLabel(load.loadStatus)}</span>
                    <small className="station-load-chart-orders">{load.remainingUnits.toLocaleString()} units · {load.openOrders} order{load.openOrders === 1 ? '' : 's'}</small>
                    <span className="station-load-tooltip" role="tooltip">
                      <strong>{load.stationName}</strong>
                      <b>{load.loadPercent}% Load</b>
                      <span>{load.remainingUnits.toLocaleString()} remaining units</span>
                      <span>{load.openOrders} active order{load.openOrders === 1 ? '' : 's'}</span>
                      <span>{load.atRiskOrders + load.criticalOrders} order{load.atRiskOrders + load.criticalOrders === 1 ? '' : 's'} at risk</span>
                      <span>{Math.round(load.currentLoadPoints).toLocaleString()} current load points</span>
                      <span>{Math.round(load.referenceLoadPoints).toLocaleString()} reference load points</span>
                      <small>Historical reference: {load.referenceMethod}</small>
                    </span>
                  </div>
                ))}
                {stationLoadRows.length === 0 ? <p>No stations available for load comparison.</p> : null}
              </div>
                </section>
              </div>
              <aside className={`work-center-load-gauge gauge-status-${workCenterLoadStatus}`} aria-label={`Overall Work Center load ${workCenterLoadPercent}%`}>
                <header>
                  <span>Work Center Load</span>
                  <strong>Overall load score</strong>
                  <small>{selectedWorkCenter?.name ?? 'No Work Center selected'}</small>
                </header>
                <div className="work-center-load-gauge-visual">
                  <svg viewBox="0 0 240 132" role="img" aria-label={`${workCenterLoadPercent}% overall load`}>
                    <path className="gauge-track low" d="M 30 108 A 90 90 0 0 1 75 30.1" />
                    <path className="gauge-track balanced" d="M 75 30.1 A 90 90 0 0 1 165 30.1" />
                    <path className="gauge-track overloaded" d="M 165 30.1 A 90 90 0 0 1 210 108" />
                    <line className="gauge-needle" x1="120" y1="108" x2={workCenterGaugeNeedleX} y2={workCenterGaugeNeedleY} />
                    <circle className="gauge-hub" cx="120" cy="108" r="8" />
                  </svg>
                  <div className="work-center-load-gauge-labels"><span>Low</span><span>Balanced</span><span>Overloaded</span></div>
                  <div className="work-center-load-gauge-score"><strong>{workCenterLoadPercent}%</strong><span>{formatTitleLabel(workCenterLoadStatus)}</span></div>
                </div>
                <dl>
                  <div><dt>Current load points</dt><dd>{Math.round(workCenterCurrentLoadPoints).toLocaleString()}</dd></div>
                  <div><dt>Historical reference</dt><dd>{Math.round(workCenterReferenceLoadPoints).toLocaleString()}</dd></div>
                  <div><dt>Loaded stations</dt><dd>{stationLoadRows.filter((station) => station.loadPercent > 0).length} / {stationLoadRows.length}</dd></div>
            ...
            <div className="station-card-grid">
              {filteredStations.map((station) => {
                ...
                const lastProductionEvent = productionEvents.find((event) => event.event_type === 'production-good' && event.station_code === station.code);
                const lastProducedOrder = lastProductionEvent?.production_order_id
                ...
                return (
                  ...
                  >
                    ...
                    <dl className="station-property-grid">
                      ...
                      <div className="station-current-client">
                        <dt>Client</dt>
                        <dd>{stationCurrentOrder?.clientName || (stationCurrentOrder ? 'Not specified' : 'No active order')}</dd>
                      </div>
      ...
      {stationLoadHelpOpen ? (
        <div className="mes-modal-backdrop station-load-help-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setStationLoadHelpOpen(false); }}>
          <section className="station-load-help-modal" role="dialog" aria-modal="true" aria-labelledby="station-load-help-title">
            <header>
              <div>
                <p className="eyebrow">Station Load</p>
                <h3 id="station-load-help-title">How current workload is calculated</h3>
                <small>A workload index based on pending production, delivery risk, order fragmentation, and each Station’s own 90-day history.</small>
              </div>
              <button type="button" aria-label="Close Station Load explanation" onClick={() => setStationLoadHelpOpen(false)}><X size={21} /></button>
            </header>

            <div className="station-load-help-content">
              <section>
                <span className="station-load-help-step">1</span>
                <div><h4>Remaining units</h4><p>Only unfinished work from open orders contributes to load. Completed, cancelled, and zero-balance orders are excluded.</p></div>
                <div className="station-load-formula"><var>R<sub>i</sub></var><b>=</b><span>max( Q<sub>i</sub> − C<sub>i</sub> − S<sub>i</sub> , 0 )</span></div>
                <small>Q = planned quantity · C = completed quantity · S = scrap quantity</small>
              </section>

              <section>
                <span className="station-load-help-step">2</span>
                <div><h4>Risk-weighted workload</h4><p>Urgent orders contribute more load using the same delivery-risk classification shown in Order Risks.</p></div>
                <div className="station-load-formula station-load-sum-formula"><var>W<sub>s</sub></var><b>=</b><span><i>Σ</i><sub>i ∈ s</sub> R<sub>i</sub> · w<sub>i</sub></span></div>
                <div className="station-load-risk-weights">
                  <span><b>Healthy</b><em>1.00×</em></span><span><b>Attention</b><em>1.10×</em></span><span><b>At risk</b><em>1.25×</em></span><span><b>Critical</b><em>1.50×</em></span>
                </div>
              </section>

              <section>
                <span className="station-load-help-step">3</span>
                <div><h4>Order fragmentation</h4><p>Several small orders create more coordination and setup pressure than one order with the same total units.</p></div>
                <div className="station-load-formula"><var>L<sub>s</sub></var><b>=</b><span>W<sub>s</sub> + 3N<sub>s</sub></span></div>
                <small>L = current load points · N = number of open orders assigned to the Station</small>
              </section>

              <section>
                <span className="station-load-help-step">4</span>
                <div><h4>Historical reference</h4><p>Each Station is compared only with its own daily load snapshots from the rolling 90-day window. P90 is the value at or below which 90% of observed loads fall, reducing distortion from exceptional outliers.</p></div>
                <div className="station-load-formula"><var>H<sub>s</sub></var><b>=</b><span>P<sub>90</sub>({`{ L`}<sub>s,t</sub> : t ∈ last 90 days {`}`})</span></div>
                <small>With fewer than 14 positive snapshots, the historical maximum is used. With no history, current load is the temporary reference.</small>
              </section>

              <section>
                <span className="station-load-help-step">5</span>
                <div><h4>Station Load percentage</h4><p>The displayed value is not capped. A Station above its historical high-load reference can exceed 100%; only the visual bar stops at full width.</p></div>
                <div className="station-load-formula station-load-fraction-formula">
                  <var>Load<sub>s</sub></var><b>=</b><span className="station-load-fraction"><i>L<sub>s</sub></i><i>H<sub>s</sub></i></span><b>× 100%</b>
                </div>
                <div className="station-load-thresholds">
                  <span className="load-no-load">0 · No Load</span><span className="load-low">1–35 · Low</span><span className="load-normal">36–65 · Normal</span><span className="load-high">66–85 · High</span><span className="load-very-high">86–100 · Very High</span><span className="load-overloaded">&gt;100 · Overloaded</span>
                </div>
              </section>
            </div>
          </section>
                ...
                return (
                  <article className="station-cycle-machine" key={station.id}>
                    ...
                    <div className="station-cycle-machine-summary" role="table" aria-label={`${station.name} accumulated time by status`}>
                      <div className="station-cycle-summary-head" role="row"><span>Status</span><span>Last Cycle Started</span><span>Last Cycle Ended</span><span>Order</span><span>Serial Number</span><span>Client</span><span>Total Time</span><span aria-hidden="true" /></div>
                      {(['running', 'idle', 'down', 'setup', 'maintenance', 'offline'] as WorkCenterStatus[]).map((status) => {
                        ...
                        const lastCycle = [...statusCycles].sort((left, right) => new Date(right.started_at).getTime() - new Date(left.started_at).getTime())[0] ?? null;
                        const lastStartedAt = lastCycle ? Math.max(new Date(lastCycle.started_at).getTime(), cycleReportRangeStart) : null;
                        const lastEndedAt = lastCycle ? Math.min(lastCycle.ended_at ? new Date(lastCycle.ended_at).getTime() : cycleClock, cycleReportRangeEnd) : null;
                        const cycleContext = getCycleContext(lastCycle);
                        return (
                          <div className="station-cycle-summary-row" role="row" key={status}>
                            ...
                            <time>{lastStartedAt === null ? '—' : formatTimestamp(new Date(lastStartedAt).toISOString())}</time>
                            <time>{lastEndedAt === null ? '—' : lastCycle && !lastCycle.ended_at && cycleReportDateRange.to === getMesOrderQuickRange('today').to ? 'Current' : formatTimestamp(new Date(lastEndedAt).toISOString())}</time>
                            <span title={cycleContext.orderNumber}>{cycleContext.orderNumber}</span>
                            <span title={cycleContext.serialNumber}>{cycleContext.serialNumber}</span>
                            <span title={cycleContext.clientName}>{cycleContext.clientName}</span>
                            <strong>{formatCycleDuration(stationStateTotals[status] ?? 0)}</strong>
                            <button type="button" disabled={!statusCycles.length} onClick={() => setCycleDetailSelection({ stationId: station.id, status })}>View All Cycles</button>
                          </div>
                        );
                      })}
                    </div>
      ...
      {cycleReportOpen && cycleDetailSelection ? (() => {
        const detailStation = selectedStations.find((station) => station.id === cycleDetailSelection.stationId) ?? null;
        const detailCycles = selectedStatusCycles
          .filter((cycle) => cycle.station_id === cycleDetailSelection.stationId && cycle.status === cycleDetailSelection.status)
          .sort((left, right) => new Date(left.started_at).getTime() - new Date(right.started_at).getTime());
        return (
          <div className="mes-modal-backdrop station-cycle-detail-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setCycleDetailSelection(null); }}>
            <section className="mes-order-modal station-cycle-detail-modal" role="dialog" aria-modal="true" aria-labelledby="station-cycle-detail-title">
              <header>
                <div>
                  <p className="eyebrow">Cycle Detail</p>
                  <h3 id="station-cycle-detail-title">{detailStation?.name ?? 'Station'} · {formatLabel(cycleDetailSelection.status)}</h3>
                  <small>{cycleReportDateRange.from} to {cycleReportDateRange.to} · {detailCycles.length} cycle{detailCycles.length === 1 ? '' : 's'}</small>
                </div>
                <button type="button" aria-label="Close cycle detail" onClick={() => setCycleDetailSelection(null)}><X size={21} /></button>
              </header>
              <div className="station-cycle-detail-table" role="table">
                <div className="station-cycle-detail-head" role="row"><span>Cycle</span><span>Started</span><span>Ended</span><span>Order</span><span>Serial Number</span><span>Client</span><span>Duration</span></div>
                {detailCycles.map((cycle, index) => {
                  const startedAt = Math.max(new Date(cycle.started_at).getTime(), cycleReportRangeStart);
                  const endedAt = Math.min(cycle.ended_at ? new Date(cycle.ended_at).getTime() : cycleClock, cycleReportRangeEnd);
                  const cycleContext = getCycleContext(cycle);
                  return (
                    <div className="station-cycle-detail-row" role="row" key={cycle.id}>
                      <strong>#{index + 1}</strong>
                      <time>{formatTimestamp(new Date(startedAt).toISOString())}</time>
                      <time>{!cycle.ended_at && cycleReportDateRange.to === getMesOrderQuickRange('today').to ? 'Current' : formatTimestamp(new Date(endedAt).toISOString())}</time>
                      <span title={cycleContext.orderNumber}>{cycleContext.orderNumber}</span>
                      <span title={cycleContext.serialNumber}>{cycleContext.serialNumber}</span>
                      <span title={cycleContext.clientName}>{cycleContext.clientName}</span>
                      <strong>{formatCycleDuration(Math.max(0, endedAt - startedAt))}</strong>
                    </div>
                  );
                })}
              </div>
