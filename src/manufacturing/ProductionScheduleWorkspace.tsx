...
type Order = { id: string; order_number: string; client_name: string | null; part_number: string; part_name: string; planned_quantity: number; completed_quantity: number; due_date: string; priority: string; status: string; assigned_station: string | null; assigned_work_center: string | null };
...
export function ProductionScheduleWorkspace({ onNavigate, organizationId }: Props) {
  const [stations, setStations] = React.useState<Station[]>([]), [workCenters, setWorkCenters] = React.useState<WorkCenter[]>([]), [orders, setOrders] = React.useState<Order[]>([]), [queue, setQueue] = React.useState<QueueItem[]>([]);
  ...
  const load = React.useCallback(async () => {
    setLoading(true);
    const [stationResult, centerResult, orderResult, queueResult] = await Promise.all([
      ...
      supabase.from('mes_production_orders').select('id, order_number, client_name, part_number, part_name, planned_quantity, completed_quantity, due_date, priority, status, assigned_station, assigned_work_center').eq('organization_id', organizationId).in('status', activeStatuses).order('due_date'),
    ...
    const loadError = stationResult.error ?? centerResult.error ?? orderResult.error ?? queueResult.error;
    if (loadError) setError(`Unable to load the production schedule: ${loadError.message}. Apply SQL migrations 134–136.`);
    else { setStations((stationResult.data ?? []) as Station[]); setWorkCenters((centerResult.data ?? []) as WorkCenter[]); setOrders((orderResult.data ?? []) as Order[]); setQueue((queueResult.data ?? []) as QueueItem[]); setError(''); }
    setLoading(false);
  }, [organizationId]);
  ...
  const availableOrders = selectedStation ? orders.filter((order) => {
    ...
    return (assignedCodes.includes(selectedStation.code) || (assignedCodes.length === 0 && Boolean(center && order.assigned_work_center === center.code))) && !queue.some((item) => item.station_id === selectedStation.id && item.production_order_id === order.id);
  }) : [];
