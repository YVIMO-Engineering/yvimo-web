...
import { type DailyProductionStat, getProductionCompliance, TEMPORARY_WEEKLY_TARGET } from './productionStatistics';

export function ProductionMetricCards({ stats }: { stats: DailyProductionStat[] }) {
  const weeklyActual = stats.reduce((total, stat) => total + stat.actualProduction, 0);
  const weeklyTarget = TEMPORARY_WEEKLY_TARGET;
  ...
  const cards = [
    ...
    { label: 'Weekly target', value: weeklyTarget.toLocaleString(), detail: '30 pieces × 7 days', icon: Target, tone: 'blue' },
    { label: 'Weekly compliance', value: compliance === null ? '—' : `${Math.round(compliance)}%`, detail: 'against 210-piece weekly target', icon: Gauge, tone: 'green' },
