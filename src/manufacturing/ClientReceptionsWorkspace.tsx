...
// Shavers are never coated, so both coating milestones are informational for them
// and delivery is reachable straight after production.
function skipsCoating(pieceType: string) {
  return ['shaver', 'shavers'].includes(pieceType.trim().toLowerCase());
}
                    ...
                    <article key={item.id}>
                      ...
                      {item.serials.length ? <div className="client-reception-serials"><header><strong>Serial numbers in this order</strong><span>{translateClientsText(languageCode, `${item.serials.filter((serial) => serial.result === 'good').length} produced · ${item.serials.length} total pieces`)}</span></header><div className="client-reception-serial-list">
                        {item.serials.map((serial) => { const isProduced = serial.result === 'good'; const isReworked = Boolean(serial.reworkedAt); return <div className={`client-reception-serial-row ${isReworked ? 'reworked' : isProduced ? 'produced' : serial.result === 'scrap' ? 'scrap' : 'pending-production'}`} key={serial.id}>
                          ...
                          <span className={`piece-stage ${skipsCoating(item.pieceType) ? 'not-required' : serial.coatingSentAt ? 'done' : ''}`}><small>Coating dispatch</small><button type="button" onClick={() => openCoatingEvidence(item, 'coating-sent', serial.id)} disabled={!isProduced || Boolean(updatingSerialKey) || Boolean(serial.coatingSentAt) || skipsCoating(item.pieceType) || selected.status === 'waiting-delivery'}>{!isProduced ? <><Clock3 size={14} /> Awaiting production</> : skipsCoating(item.pieceType) ? <><Check size={14} /><span><b>Not required</b></span></> : serial.coatingSentAt ? <><Check size={14} /><span><b>Sent</b><time>{formatReceptionTimestamp(serial.coatingSentAt, languageCode)}</time></span></> : <><Send size={14} /> Send to Coating</>}</button></span>
                          <span className={`piece-stage ${skipsCoating(item.pieceType) ? 'not-required' : serial.coatingReturnedAt ? 'done' : ''}`}><small>Coating return</small><button type="button" onClick={() => openCoatingEvidence(item, 'coating-returned', serial.id)} disabled={!isProduced || Boolean(updatingSerialKey) || !serial.coatingSentAt || Boolean(serial.coatingReturnedAt) || skipsCoating(item.pieceType) || selected.status === 'waiting-delivery'}>{!isProduced ? <><Clock3 size={14} /> Awaiting production</> : skipsCoating(item.pieceType) ? <><Check size={14} /><span><b>Not required</b></span></> : serial.coatingReturnedAt ? <><Check size={14} /><span><b>Received</b><time>{formatReceptionTimestamp(serial.coatingReturnedAt, languageCode)}</time></span></> : <><RotateCcw size={14} /> Receive Coating</>}</button></span>
                          <span className={`piece-stage delivery ${serial.sentAt ? 'done' : ''}`}><small>Delivery</small><button type="button" onClick={() => void openCoatingEvidence(item, 'sent', serial.id)} disabled={!isProduced || Boolean(updatingSerialKey) || (!serial.coatingReturnedAt && !skipsCoating(item.pieceType) && selected.status !== 'waiting-delivery') || Boolean(serial.sentAt)}>{!isProduced ? <><Clock3 size={14} /> Awaiting production</> : serial.sentAt ? <><Check size={14} /><span><b>Sent</b><time>{formatReceptionTimestamp(serial.sentAt, languageCode)}</time></span></> : <><Truck size={14} /> Send</>}</button></span>
