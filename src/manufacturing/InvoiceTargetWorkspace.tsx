  ...
  return <section className="invoice-target-workspace">
    ...
    <div className="invoice-target-layout">
      ...
      <main className="invoice-annual-column"><section className="invoice-annual-panel"><header><div><span>ANNUAL PERFORMANCE · TOTAL INVOICING</span><h2>{year} invoicing overview</h2><p>Every line represents one workcenter. Empty months remain unconnected until a report is uploaded.</p></div><div><small>{selectedName}</small><strong>{money.format(annualTotal)}</strong><span>selected annual total</span></div></header><AnnualChart workCenters={workCenters} reports={reports} /><footer><span><i /> Values use “Total” (including tax) from the CONTPAQ Total General row.</span><strong>{Object.keys(reports).length} monthly reports loaded</strong></footer></section>
        <section className="invoice-division-summary"><header><div><span>ORGANIZATION SUMMARY</span><h2>Annual invoicing by division</h2><p>Consolidated totals for every workcenter with reports in {year}.</p></div><strong>{money.format(organizationTotal)}</strong></header><div className="invoice-summary-table-wrap"><table><thead><tr><th>Division / Workcenter</th><th>Code</th><th>Months reported</th><th>Share</th><th>Annual invoicing</th></tr></thead><tbody>{divisionTotals.map((center) => <tr key={center.code}><td>{center.name}</td><td>{center.code}</td><td>{center.reportedMonths} / 12</td><td><span className="invoice-share"><i style={{ width: `${organizationTotal ? center.amount / organizationTotal * 100 : 0}%` }} /></span><b>{organizationTotal ? (center.amount / organizationTotal * 100).toFixed(1) : '0.0'}%</b></td><td>{money.format(center.amount)}</td></tr>)}{!divisionTotals.length ? <tr><td className="invoice-summary-empty" colSpan={5}>No reports have been uploaded for this year.</td></tr> : null}</tbody><tfoot><tr><th colSpan={2}>All workcenters</th><th>{Object.keys(reports).length} / {workCenters.length * 12} reports</th><th>{organizationTotal ? '100%' : '0%'}</th><th>{money.format(organizationTotal)}</th></tr></tfoot></table></div></section>
      </main>
    </div>
  </section>;
}
