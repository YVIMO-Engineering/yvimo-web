...
import { RemissionsWorkspace } from './manufacturing/RemissionsWorkspace';
import { InvoicesWorkspace } from './manufacturing/InvoicesWorkspace';
import { OtcReconciliationWorkspace } from './manufacturing/OtcReconciliationWorkspace';
...
const translations: Record<Exclude<LanguageCode, 'en'>, Record<string, string>> = {
  es: {
    ...
    'Sort by medical record number': 'Ordenar por número de expediente',
    Filters: 'Filtros',
    'Lowest to highest': 'Menor a mayor',
    'Highest to lowest': 'Mayor a menor',
    'Patient table pages': 'Páginas de la tabla de pacientes',
    Page: 'Página',
    of: 'de',
    Previous: 'Anterior',
    Next: 'Siguiente',
    ...
    'Full name, CURP, and medical record number are required.': 'Nombre completo, CURP y número de expediente son obligatorios.',
    'CURP does not contain a valid sex marker.': 'La CURP no contiene un indicador de sexo válido.',
    'From CURP': 'De la CURP',
    ...
    'This CURP is already registered in this organization.': 'Esta CURP ya está registrada en esta organización.',
    'This medical record number is already registered in this organization.': 'Este número de expediente ya está registrado en esta organización.',
    'This CURP and this medical record number are both already registered in this organization.': 'Esta CURP y este número de expediente ya están registrados en esta organización.',
    'Already registered to': 'Ya registrado en',
...
function LoggedDashboardPage({ user, onSignOut, onNavigate, onUpdateAvatar, activePath, t, languageCode, standaloneHealth = false, publicHealth = false, onToggleLanguage }: { user: AppUser; onSignOut: () => void; onNavigate: (path: string) => void; onUpdateAvatar: (file: File) => Promise<AvatarUploadResult>; activePath: string; t: Translator; languageCode: LanguageCode; standaloneHealth?: boolean; publicHealth?: boolean; onToggleLanguage?: () => void }) {
  ...
  const intelligenceModules: Array<{
    ...
    {
      label: 'Import Orders',
      description: 'Create import orders, calculate landed costs, validate margins, and maintain a traceable commercial history.',
      icon: Container,
  ...
  const activeOtcSection = activePath.endsWith('/otc/purchase-orders') ? 'purchase-orders' : activePath.endsWith('/otc/remissions') ? 'remissions' : activePath.endsWith('/otc/invoices') ? 'invoices' : activePath.endsWith('/otc/reconciliation') ? 'reconciliation' : 'order-to-cash';
  ...
  const renderActiveMesWorkspace = () => {
    ...
    if (isOtcSection) {
      ...
      if (activeOtcSection === 'remissions') {
        return <RemissionsWorkspace onNavigate={onNavigate} organizationId={activeManufacturingOrganizationId} />;
      }
      if (activeOtcSection === 'invoices') {
        return <InvoicesWorkspace onNavigate={onNavigate} organizationId={activeManufacturingOrganizationId} />;
      }
      if (activeOtcSection === 'reconciliation') {
        return <OtcReconciliationWorkspace onNavigate={onNavigate} organizationId={activeManufacturingOrganizationId} />;
      }
    ...
    if (activePath === '/workspace/manufacturing-ops/mes/quotations') {
      return <QuotationsWorkspace onNavigate={onNavigate} organizationId={activeManufacturingOrganizationId} organizationName={manufacturingOrganization?.name ?? 'Manufacturing Organization'} organizationLogoUrl={manufacturingOrganization?.logoUrl ?? ''} />;
    }
      ...
      {isOtcSection ? (
        <aside className="supplier-shell-context-menu otc-shell-context-menu" aria-label="OTC sections">
          ...
          <nav>
            ...
            <button type="button" className={activeOtcSection === 'reconciliation' ? 'active' : ''} aria-current={activeOtcSection === 'reconciliation' ? 'page' : undefined} onClick={() => onNavigate('/workspace/manufacturing-ops/intelligence/otc/reconciliation')}>
              <Scale size={18} />
            ...
            <button type="button" className={activeOtcSection === 'remissions' ? 'active' : ''} aria-current={activeOtcSection === 'remissions' ? 'page' : undefined} onClick={() => onNavigate('/workspace/manufacturing-ops/intelligence/otc/remissions')}>
              <Truck size={18} />
              <span>Remissions</span>
            </button>
            <button type="button" className={activeOtcSection === 'invoices' ? 'active' : ''} aria-current={activeOtcSection === 'invoices' ? 'page' : undefined} onClick={() => onNavigate('/workspace/manufacturing-ops/intelligence/otc/invoices')}>
              <Receipt size={18} />
              <span>Invoices</span>
            </button>
          </nav>
